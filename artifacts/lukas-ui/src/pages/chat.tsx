...
import {
  ...
  Paperclip, X, FileText, Film, ImageIcon, File as FileIcon, ArrowDown,
} from "lucide-react";
...
/** Macht URLs anklickbar, statt sie als toten Text stehen zu lassen. */
...
function Avatar({ who }: { who: "lukas" | "issa" }) {
  return who === "lukas" ? (
    <div className="w-7 h-7 rounded-full bg-primary/15 border border-primary/25 flex items-center justify-center text-[11px] font-semibold text-primary shrink-0">
      L
    </div>
  ) : (
    <div className="w-7 h-7 rounded-full bg-secondary flex items-center justify-center text-[11px] font-semibold text-muted-foreground shrink-0">
      I
    </div>
  );
}
...
export default function Chat() {
   ...
   * Scrollen.
   *
   * Vorher lag der Verlauf in einem Radix-ScrollArea und wurde ueber
   * scrollIntoView() auf ein leeres Div am Ende bewegt. Zwei Probleme: der
   * eigentliche Scroll-Container ist bei Radix ein inneres Viewport-Element,
   * weshalb scrollIntoView gern den falschen Vorfahren bewegt — und in den
   * Abhaengigkeiten fehlte optimisticMsg. Genau die EIGENE Nachricht hat also
   * kein Scrollen ausgeloest.
   *
   * Jetzt ein normaler Scroll-Container mit eigener Referenz, den wir direkt
   * ans Ende setzen. Zusaetzlich das Verhalten echter Chat-Apps: automatisch
   * gescrollt wird nur, wenn man ohnehin unten steht. Wer nach oben gescrollt
   * hat und liest, wird nicht weggerissen — beim eigenen Absenden aber immer.
   */
  ...
  const handleScroll = useCallback(() => {
    const el = scrollRef.current;
    if (!el) return;
    // 80px Toleranz: waehrend eine Antwort einlaeuft, wandert der Boden — ohne
    // Toleranz waere man staendig "nicht ganz unten".
    setAtBottom(el.scrollHeight - el.scrollTop - el.clientHeight < 80);
  }, []);

  useLayoutEffect(() => {
    if (atBottom) scrollToBottom("auto");
    // optimisticMsg gehoert hier zwingend rein — das war der eigentliche Bug.
  }, [activeConv?.messages, optimisticMsg, streamContent, streaming, atBottom, scrollToBottom]);

  // Gespraechswechsel: ohne Animation ans Ende, wie beim Oeffnen eines Chats.
  useLayoutEffect(() => {
    setAtBottom(true);
    scrollToBottom("auto");
  ...
  // Eingabefeld waechst mit dem Text, wie bei Claude/ChatGPT.
  useLayoutEffect(() => {
  ...
  const handleNew = async () => {
    const res = await createConvo.mutateAsync({
      data: { title: `Chat ${new Date().toLocaleTimeString("de-DE", { hour: "2-digit", minute: "2-digit" })}` },
    });
    setActiveId(res.id);
  ...
  const handleSend = useCallback(async () => {
    ...
    setStreaming(true);
    setStreamContent("");
    // Beim eigenen Absenden immer ans Ende, egal wo man vorher stand.
    setAtBottom(true);
  ...
  const autoVoice =
    typeof window !== "undefined" && new URLSearchParams(window.location.search).get("voice") === "auto";
        ...
        {showList && (
          <div className="w-full md:w-72 border-r border-border flex flex-col shrink-0">
            ...
            <div className="flex-1 overflow-y-auto px-2 pb-3 space-y-0.5">
              {loadingConvos && (
                <div className="text-sm text-muted-foreground px-3 py-2">Lädt…</div>
              )}
              {convos.map((c) => (
                <div
                  key={c.id}
                ...
                >
                  ...
                  <button
                    ...
                    className="opacity-0 group-hover:opacity-100 focus:opacity-100 transition-opacity text-muted-foreground hover:text-destructive p-1 -m-1"
              ...
              {!loadingConvos && convos.length === 0 && (
                <p className="text-sm text-muted-foreground px-3 py-2">Noch keine Gespräche.</p>
              )}
            </div>
          </div>
        )}
          ...
          <div className="flex-1 flex flex-col min-w-0 relative">
            {activeId ? (
              <>
                        ...
                        <div key={m.id} className={`flex gap-3 min-w-0 ${own ? "justify-end" : ""}`}>
                          ...
                          <div className={`min-w-0 ${own ? "max-w-[85%]" : "flex-1"}`}>
                            <div
                              className={
                                own
                                  ? "bg-secondary rounded-2xl rounded-tr-md px-4 py-3"
                                  : ""
                              }
                            >
                                  ...
                                  {files.map((a) => {
                                    ...
                                    return a.kind === "image" ? (
                                      ...
                                      <a
                                        ...
                                        target="_blank"
                                        rel="noreferrer"
                                        className="flex items-center gap-2 rounded-lg border border-border bg-card px-3 py-2 text-sm hover:border-primary/40 transition-colors"
                                      >
                            ...
                            <div
                              className={`text-xs text-muted-foreground/60 mt-1.5 ${own ? "text-right" : ""}`}
                            >
                    ...
                    {optimisticMsg && (
                      <div className="flex gap-3 justify-end min-w-0">
                        <div className="min-w-0 max-w-[85%]">
                          ...
                          <div className="text-xs text-muted-foreground/60 mt-1.5 text-right flex items-center justify-end gap-1.5">
                ...
                {/* Sprung nach unten, wenn man weiter oben liest */}
                {!atBottom && (
                  <button
                    onClick={() => {
                      setAtBottom(true);
                    ...
                    className="absolute bottom-32 left-1/2 -translate-x-1/2 z-10 flex items-center gap-1.5 rounded-full border border-border bg-card px-3 py-1.5 text-xs shadow-lg hover:border-primary/40 transition-colors"
                  >
                ...
                {/* ── Eingabe ──────────────────────────────────────────── */}
                <div
                  className="px-4 pb-4 pt-2 sm:px-6 shrink-0"
                ...
                >
                  <div className="max-w-3xl mx-auto">
                      ...
                      <div className="mb-2 flex flex-wrap gap-2">
                        {pending.map((a) => {
                          ...
                          return (
                            <div
                              key={a.id}
                              className="flex items-center gap-2 bg-card border border-border rounded-xl pl-2 pr-1 py-1.5 max-w-[240px]"
                            >
                              {a.kind === "image" ? (
                                <img
                                  src={`${BASE}${a.url}`}
                    ...
                    {/* Eine Fläche, in der alles sitzt — wie bei Claude/ChatGPT */}
                    <div className="flex items-end gap-2 rounded-2xl border border-border bg-card px-2 py-2 focus-within:border-primary/40 transition-colors">
                      ...
                      <button
                        ...
                        className="p-2 rounded-lg text-muted-foreground hover:text-foreground hover:bg-secondary transition-colors disabled:opacity-40 shrink-0"
                      ...
                      <textarea
                        ...
                        rows={1}
                        disabled={streaming}
                        className="flex-1 bg-transparent resize-none py-2 text-[15px] leading-6 outline-none placeholder:text-muted-foreground/60 disabled:opacity-50 max-h-[200px]"
                      />
                      <button
                        onClick={handleSend}
                        disabled={(!input.trim() && pending.length === 0) || streaming}
                        className="p-2 rounded-lg bg-primary text-primary-foreground disabled:opacity-30 disabled:bg-secondary disabled:text-muted-foreground transition-colors shrink-0"
                        aria-label="Senden"
                      >
                        {streaming ? (
              ...
              <div className="flex-1 flex flex-col items-center justify-center text-center p-8">
                ...
                <p className="text-muted-foreground text-sm mb-6 max-w-sm">
                  Starte ein neues Gespräch — Lukas erinnert sich an alles, was ihr besprecht.
                </p>
