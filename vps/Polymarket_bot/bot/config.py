...
MIN_EDGE_MAKER = 0.45         # 45% minimum edge (Gemini_prob - market_price) required — blocks near-limit entries
...
MIN_EDGE_TAKER = 0.70         # 70% edge required — effectively disables taker/FOK orders

# PATCH-POLY-PRICECAP-V1: Hard structural caps on BUY price.
# Trades at >=0.95 produce ~1% upside vs ~99% downside (mathematical suicide
# when win-rate < 99%). Trades at <=0.05 are similarly capped on the NO side.
# These guards apply ONLY to opening BUYs; SL/TP sells must work at any price.
# Override via env vars MAX_BUY_PRICE / MIN_BUY_PRICE if ever needed.
MAX_BUY_PRICE = float(os.getenv("MAX_BUY_PRICE", "0.85"))
MIN_BUY_PRICE = float(os.getenv("MIN_BUY_PRICE", "0.15"))
...
GEMINI_MIN_CONFIDENCE = float(os.getenv("GEMINI_MIN_CONFIDENCE", "0.99999"))
...
# Kelly model
KELLY_FRACTION = 0.10         # Fractional Kelly (lambda) — used internally, capped by BET_SIZE_PCT
KELLY_MAX_FRACTION = 0.0002   # Hard cap: max 0.02% of bankroll per single trade
...
MIN_BET_SIZE = float(os.getenv("MIN_BET_SIZE", "0.10"))
...
SL_RATIO = 0.08               # Stop-loss at -8% loss (GROWTH phase only)
SL_RATIO_CATASTROPHIC = 0.015  # Emergency SL at -1.5% loss regardless of phase
...
AI_GATE_CONFIDENCE_THRESHOLD = float(os.getenv("AI_GATE_CONFIDENCE_THRESHOLD", "0.99999"))
...
# Dynamic position cap — scales with bankroll, replaces static MAX_OPEN_TRADES.
# Tuneable via env: TARGET_CAPITAL_PER_SLOT (default 5.0), MAX_OPEN_TRADES_FLOOR (10),
# MAX_OPEN_TRADES_CEILING (50). Profitable bot => more concurrent positions.
def dynamic_max_positions(bankroll: float) -> int:
    target  = float(os.getenv("TARGET_CAPITAL_PER_SLOT", "5.0"))
    floor   = int(os.getenv("MAX_OPEN_TRADES_FLOOR", "10"))
    ceiling = int(os.getenv("MAX_OPEN_TRADES_CEILING", "50"))
    try:
        bk = float(bankroll)
    except Exception:
        bk = 0.0
    base = max(bk, 50.0)  # never use < $50 as denominator
    raw = int(base / target)
    cap = max(floor, min(ceiling, raw))
    btc_reserve = int(os.getenv("BTC_RESERVE_SLOTS", "1"))
    return max(1, cap - btc_reserve)
