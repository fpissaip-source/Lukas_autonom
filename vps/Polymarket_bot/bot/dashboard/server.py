...
<!-- TASK_65: Speed / Latency panel -->
<div id="speedup-panel" style="margin:16px;padding:12px;border:1px solid #2a3550;border-radius:8px;background:#0f1626;color:#cbd5e1;font-family:monospace;font-size:13px">
  <h3 style="margin:0 0 8px 0;color:#7dd3fc">⚡ Speed / Latency (Task #65)</h3>
  <div id="speedup-body">loading…</div>
</div>
<script>
async function _t65refresh(){
  try{
    const r = await fetch("/api/state",{cache:"no-store"});
    const d = await r.json();
    const s = d.speedup || {};
    const lat = s.latency || {};
    const meta = s.meta_cache || {};
    const ws = s.ws_orderbook || {};
    document.getElementById("speedup-body").innerHTML =
      "<div>WS: " + (ws.connected ? "<span style=\"color:#34d399\">●connected</span>" : "<span style=\"color:#f87171\">●down</span>") +
      " subs=" + (ws.subscribed_tokens || 0) +
      " books=" + (ws.books_in_memory || 0) +
      " snaps=" + (ws.snapshots || 0) +
      " deltas=" + (ws.deltas || 0) + "</div>" +
      "<div>Meta-cache: " + (meta.size || 0) + " entries (hits=" + (meta.hits || 0) +
      " miss=" + (meta.misses || 0) + " hit_rate=" + ((meta.hit_rate||0).toFixed(2)) + ")</div>" +
      "<div>Latency p50=" + (lat.p50_ms || "–") + "ms p90=" + (lat.p90_ms || "–") +
      "ms p99=" + (lat.p99_ms || "–") + "ms (n=" + (lat.count || 0) + ")</div>";
  } catch(e){
    document.getElementById("speedup-body").innerText = "err: " + e;
  }
}
setInterval(_t65refresh, 3000);
_t65refresh();
</script>
...
def _load_polymarket_reality() -> dict:  # # REALITY_TASK_64_PANEL: loader
...
class DashboardHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path in ("/", "/index.html"):
            ...
            # # REALITY_TASK_64_PANEL: state_inject
            try:  # TASK_65: speedup status (latency / meta cache / ws orderbook)
                state["speedup"] = _bot_speedup_t65.status() if _bot_speedup_t65 else None
            except Exception as _e:
                state["speedup"] = {"error": str(_e)}
