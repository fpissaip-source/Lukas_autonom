...
import time
import logging
from trading import db_writer
from trading import reality  # REALITY_TASK_64: import
...
from learning.trade_learner import TradeLearner  # adaptive feedback
...
from data.intel_bridge import get_market_intel, log_intel_check  # Lukas intel bridge
...
class ArbitrageBot:
    def __init__(self, dry_run: bool = False):
        ...
        self._last_reality_sync = 0.0  # REALITY_TASK_64: init
        ...
        for order_id, pos in list(self._live_positions.items()):
            ...
            if not buy_filled:
                ...
                if filled_shares >= 5.0:
                    ...
                    tp_price = min(tp_price, 0.999)  # Polymarket price cap
            ...
            if reason:
                ...
                if tp_already_filled:
                    ...
                    try:
                        db_writer.record_close(order_id=order_id,
                                               exit_price=entry_price * (1.0 + TP_RATIO),
                                               payout_usd=received, pnl_usd=profit,
                                               status="win" if profit >= 0 else "loss")
                    except Exception as _dbe:
                        logger.warning(f"[DB] record_close(tp) failed: {_dbe}")
                    to_remove.append(order_id)
                    self.kelly.release(entry_size)
                    self._save_bankroll()
                    continue
                ...
                if best_bid <= DEAD_BID:
                    ...
                    try:
                        ...
                        pass
                    # REALITY_TASK_64: full_loss
                    try:
                        db_writer.record_close(order_id=order_id, exit_price=0.01,
                                               payout_usd=0.0, pnl_usd=-entry_size,
                                               status="loss")
                    except Exception as _dbe:
                        logger.warning(f"[DB] record_close(dead) failed: {_dbe}")
                    to_remove.append(order_id)
                    self.kelly.release(entry_size)
                    self._save_bankroll()
                    continue
                ...
                if sell_order_id == "BALANCE_ERROR":
                    ...
                    if time_to_expiry > 60:
                        ...
                        try:
                            db_writer.record_close(order_id=order_id, exit_price=0.0,
                                                   payout_usd=0.0, pnl_usd=-entry_size,
                                                   status="loss")
                        except Exception as _dbe:
                            logger.warning(f"[DB] record_close(bal) failed: {_dbe}")
                        to_remove.append(order_id)
                        self.kelly.release(entry_size)
                        self._save_bankroll()
                        continue
                ...
                if remaining <= DUST:
                    ...
                    try:
                        db_writer.record_close(order_id=order_id,
                                               ...
                                               payout_usd=realized_value,
                                               pnl_usd=realized_pnl,
                                               status="win" if realized_pnl >= 0 else "loss")
                    except Exception as _dbe:
                        logger.warning(f"[DB] record_close(sell) failed: {_dbe}")
                    to_remove.append(order_id)
                    self.kelly.release(entry_size)
                    self._save_bankroll()
    ...
    def _refresh_markets(self, now: float):
        ...
        if new_count:
            ...
            # TASK_65: notify speedup so the WS subscriber re-subscribes to
            # newly registered token_ids and the meta cache pre-warms them.
            try:
                from trading import _speedup as _t65_speedup
                _t65_speedup.on_markets_refreshed(self)
            except Exception as _e:
                logger.debug(f"[TASK_65] on_markets_refreshed swallowed: {_e}")
    ...
    def run(self):
        ...
        # TASK_65: install sub-400ms execution speedups (latency telemetry,
        # CLOB metadata cache, websocket orderbook subscriber). Direct
        # in-source integration (replaces earlier patch_bot_speed.py
        # bootstrap script).
        try:
            from trading import _speedup as _t65_speedup
            _t65_speedup.install()
            _t65_speedup.init_for_bot(self)
        except Exception as _e:
            logger.warning(f"[TASK_65] speedup install failed: {_e}")
        ...
        for market_id, state in _sorted_markets:
            ...
            if state.is_event and state.question:
                ...
                if gemini_prob is not None:
                    ...
                    if gemini_conf < (self._learner.adaptive_min_conf(GEMINI_MIN_CONFIDENCE) if getattr(self, '_learner', None) else GEMINI_MIN_CONFIDENCE):  # adaptive_min_conf
            ...
            # --- 3e. Lukas Intel Bridge (Reddit + Moltbook cross-reference) ---
            _intel = None
            try:
                if state.is_event and state.question:
                    _intel = get_market_intel(market_id, state.question, gemini_q=q)
                    if _intel.has_intel():
                        logger.info(
                            f"[INTEL:{market_id[:20]}] {_intel.summary_line()} "
                            f"q={q:.3f} p={p_yes:.3f}"
                        )
                        if _intel.boost_applied:
                            q = max(0.01, min(0.99, q + _intel.boost_applied))
                            logger.info(
                                f"[INTEL_BOOST:{market_id[:20]}] applied {_intel.boost_applied:+.4f} "
                                f"→ q={q:.3f}"
                            )
            except Exception as _ie:
                logger.warning(f"[INTEL] bridge failed for {market_id[:20]}: {_ie}")
            ...
            try:
                if _intel is not None and _intel.has_intel():
                    log_intel_check(
                        _intel, gemini_q=q, gemini_conf=gemini_conf,
                        market_p=p_yes, edge_ev=edge_result.ev_net,
                        notes=("has_edge" if edge_result.has_edge else "no_edge"),
                    )
            except Exception:
                pass
            ...
            if kelly_result.is_viable and kelly_result.position_size > 0:
                ...
                proposal = TradeProposal(
                    ...
                    max_positions=dynamic_max_positions(self.kelly.bankroll),
    ...
    def _execute_opportunities(self, opportunities: list[TradeOpportunity]):
        ...
        for opp in opportunities:
            ...
            if not self.dry_run:
                ...
                # ===== TRADING PAUSE GATE =====
                if os.environ.get("POLYMARKET_PAUSED", "0") == "1":
                    logger.info(f"[PAUSED] {opp.market_id}: POLYMARKET_PAUSED=1 — skipping new entry")
                    continue
                # Position slot limits — event markets only
                n_open = len(self._live_positions)
                _dyn_cap = dynamic_max_positions(self.kelly.bankroll)
                if n_open >= _dyn_cap:
                    logger.info(
                        f"[SKIP] {opp.market_id}: {n_open}/{_dyn_cap} positions — full (bankroll-scaled)"
                    )
                    continue
                # MAX_TRADES_EVENT now follows the same dynamic cap (was static 10).
                if n_open >= _dyn_cap:
                    logger.info(
                        f"[SKIP] {opp.market_id}: {n_open}/{_dyn_cap} event slots used (bankroll-scaled)"
                    )
                    continue
            ...
            if False and side == "BOTH":  # dead code, kept for reference
                ...
                if (market.token_id_yes in open_token_ids_arb
                    ...
                    continue
                if os.environ.get("POLYMARKET_PAUSED", "0") == "1":
                    logger.info(f"[PAUSED] {opp.market_id}: POLYMARKET_PAUSED=1 — skipping arb")
                    continue
                n_open_arb = len(self._live_positions)
                _dyn_cap_arb = dynamic_max_positions(self.kelly.bankroll)
                if n_open_arb + 2 > _dyn_cap_arb:
                    logger.info(
                        f"[SKIP] {opp.market_id}: arb needs 2 slots, only "
                        f"{_dyn_cap_arb - n_open_arb} free (cap={_dyn_cap_arb})"
                    )
                    continue
                if self.kelly.available_capital < size:
                    logger.info(
    ...
    def _record_trade(self, market_id: str, side: str, size: float, price: float,
        ...
        if not self.dry_run and order_id and token_id:
            ...
            self._save_live_positions()
            try:
                _mq = self._markets.get(market_id)
                _question = getattr(_mq, "question", "") if _mq else ""
                db_writer.record_open(
                    market_id=market_id, market_question=_question,
                    side=side, size_usd=size, price=price, order_id=order_id,
                    token_id=token_id, tick_size=tick_size, neg_risk=neg_risk,
                    end_time=end_time,
                )
            except Exception as _dbe:
                logger.warning(f"[DB] record_open failed: {_dbe}")
            logger.info(
