...
# PATCH-POLY-PRICECAP-V1
try:
    from config import MAX_BUY_PRICE, MIN_BUY_PRICE
except Exception:
    MAX_BUY_PRICE, MIN_BUY_PRICE = 0.85, 0.15
...
class OrderExecutor:
    ...
    def place_order(
        ...
        # PATCH-POLY-PRICECAP-V1: structural BUY-price caps.
        # Blocks the killer-trade class identified in the Apr 21 killswitch postmortem
        # (71 trades at >=0.95, 1% upside vs 99% downside, 6.7% WR -> wipeout).
        if side.upper() == "BUY":
            if price >= MAX_BUY_PRICE:
                logger.warning(
                    f"[PRICE-CAP-BLOCK] Refusing BUY @ {price:.4f} >= {MAX_BUY_PRICE} "
                    f"(structural payout asymmetry — would need >{MAX_BUY_PRICE*100:.0f}% true WR to break even)"
                )
                return None
            if price <= MIN_BUY_PRICE:
                logger.warning(
                    f"[PRICE-CAP-BLOCK] Refusing BUY @ {price:.4f} <= {MIN_BUY_PRICE} "
                    f"(too far OTM to overcome spread/fees)"
                )
                return None
