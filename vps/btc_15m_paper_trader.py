# AUTO-GENERATED PAPER VARIANT — do not edit live trader from here.
#!/usr/bin/env python3
"""
...
STATE_FILE = BASE_DIR / "btc_paper_state.json"
LOG_FILE = BASE_DIR / "btc_paper_trader.log"
...
DRAWDOWN_STATE_FILE = BASE_DIR / "btc_paper_drawdown_state.json"
...
def paper_telegram(msg):
    """PAPER: only forward important events; mute decision noise."""
    try:
        if not msg:
            return
        important = ("WIN!", "LOSS.", "started", "stuck", "Paused", "killswitch", "Heartbeat", "PAPER")
        if any(k in msg for k in important):
            telegram("🧪 [PAPER] " + msg)
        else:
            log(f"[PAPER-TG-MUTED] {msg[:140]}")
    except Exception as _e:
        log(f"paper_telegram error: {_e}")
...
def get_available_balance():
    ...
    cash_file = BASE_DIR / "btc_paper_cash.json"
    try:
        if cash_file.exists():
    ...
    for pos in positions:
        ...
        try:
                    ...
                    try:
                        ...
                        if max(pu, pd_) >= 0.95:
                            ...
                            if time_since_end >= STUCK_ALERT_AFTER:
                                ...
                                paper_telegram(f"⚠ BTC 5m position stuck: {slug} | {time_since_end//60}m past window end | UP={pu:.2f} DOWN={pd_:.2f}")
                            still_open.append(pos); continue
            ...
            if won:
                ...
                if _pgdb:
                    try:
                        _pgdb.close_trade_by_slug("btc_5m_paper", slug, status="win",
                            exit_price=(price_up if direction == "Up" else price_down),
                            payout=payout, pnl=profit, raw_update={"shares": shares})
                        _pgdb.record_bankroll(bot="btc_5m_paper", balance=new_cash, pnl=state["total_pnl"], note="post-win")
                    except Exception as _e:
                        log(f"[db] win hook failed: {_e}")
                paper_telegram(f"🟢 BTC 5m WIN! {direction} [{_btc_window_label(market, slug)}] | Payout ${payout:.2f} (+${profit:.2f}) | Cash ${new_cash:.2f}")
            else:
                ...
                if _pgdb:
                    try:
                        _pgdb.close_trade_by_slug("btc_5m_paper", slug, status="loss",
                            exit_price=(price_up if direction == "Up" else price_down),
                            payout=0, pnl=-stake)
                        _pgdb.record_bankroll(bot="btc_5m_paper", balance=current_cash, pnl=state["total_pnl"], note="post-loss")
                    except Exception as _e:
                        log(f"[db] loss hook failed: {_e}")
                paper_telegram(f"🔴 BTC 5m LOSS. {direction} [{_btc_window_label(market, slug)}] | Lost ${stake:.2f} | Cash ${current_cash:.2f}")
        except Exception as e:
...
def place_trade(client, market, direction, stake):
    """PAPER: simulate immediate fill at mid price (AGGRO_SPREAD=0). No CLOB call."""
    AGGRO_SPREAD = 0.00  # paper trades at mid for cleanest edge measurement

    if direction == "Up":
        token_id = market["token_up"]
        market_price = market["price_up"]
    else:
        token_id = market["token_down"]
        market_price = market["price_down"]

    buy_price = min(round(market_price + AGGRO_SPREAD, 2), 0.99)
    if buy_price < 0.01:
        return None, f"market price too low: ${market_price}"
    ...
    if actual_cost > balance:
        ...
        actual_cost = size * buy_price
    if actual_cost > balance:
        return None, f"Insufficient balance: ${balance:.2f} < ${actual_cost:.2f}"

    fake_order_id = "paper_" + hex(int(time.time() * 1000))[2:]
    log(f"[PAPER] FILLED: BUY {direction} | {size} shares @ ${buy_price} (mid: ${market_price}) = ${actual_cost:.2f} | order={fake_order_id}")
    resp = {"success": True, "orderID": fake_order_id, "status": "matched", "size": str(size), "price": str(buy_price)}
...
def run_cycle():
    ...
    if confidence >= 75:
        ...
        stake = kelly_base * 0.8
    elif confidence >= 56:
        stake = kelly_base * 0.6
    elif confidence >= 50:
        stake = kelly_base * 0.4
    else:
        log(f"[PAPER] Confidence {confidence:.1f}% < 50% floor — skipping trade.")
        return "skipped" 
    ...
    try:
        client = None  # PAPER: no CLOB client
        ...
        if success:
            ...
            if _pgdb:
                try:
                    _pgdb.insert_trade(
                        bot='btc_5m_paper', market_slug=market['slug'],
            ...
            paper_telegram(
                f"{'📈' if direction == 'Up' else '📉'} BTC 5m: <b>{direction}</b>\n"
                f"Stake: ${actual_cost:.2f} | {analysis}\n"
                f"Market: {market['question']}\n"
                f"Win streak: {state['consecutive_wins']} | Total PnL: ${state['total_pnl']:.2f}"
            )
...
def main():
    log("=" * 60)
    log("BTC 5m PAPER Trader — shadow strategy (no real orders) | floor=50% | aggro=0 | max=5/window")
    ...
    MAX_TRADES_PER_WINDOW = int(os.environ.get("MAX_TRADES_PER_WINDOW", "5"))
